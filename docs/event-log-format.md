...
| 10    | `MainAutoRebased` | Emitted by ShipPhase on every auto-rebase attempt of local main onto origin/main (DivergedNoConflict path). Emitted regardless of outcome. Not emitted when `--no-auto-merge` is set (no attempt is made). |
...
| `MainAutoRebased` | `from_sha`, `onto_sha`, `local_commits_replayed`, `outcome` | `{"from_sha": "<sha>", "onto_sha": "<sha>", "local_commits_replayed": ["sha1", "sha2"], "outcome": "clean"}`. `from_sha` is the HEAD of local main before the rebase. `onto_sha` is the SHA of origin/main at the time of the attempt. `local_commits_replayed` is the list of SHAs that were ahead of origin/main on local main (newest first, may be empty). `outcome` is `"clean"` on success or `"raced_to_conflict"` when the divergence probe predicted no conflict but the rebase found one. Phase is always `Ship` (= 3). |
