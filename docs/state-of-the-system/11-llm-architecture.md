...
Last refreshed: 2026-06-14 (`heartbeat-stage-07-cutover`, Stage 07 cutover; documents the interactive-hook transport becoming the loaded-config default and the `ClaudeCodePreflight` capability gate, on top of the Stage 05-06 process-hardening + completion-redesign code surface. Dogfood validation of the cutover is operator-run and pending; see [docs/heartbeat/evidence/stage-07-dogfood.md](../heartbeat/evidence/stage-07-dogfood.md))
...
|---|---|---|---|---|---|
| `ClaudeCodeAgent` | `claude-code` / `anthropic` | Default `interactive-hook` transport: interactive Claude under a terminal host (no `--print`); `print` (`claude --print --verbose --output-format stream-json [...]`, brief on stdin) is the rollback | removes `ANTHROPIC_API_KEY`, sets `CLAUDE_CODE_MAX_OUTPUT_TOKENS` | interactive: completion synthesized from the persisted transcript; print: NDJSON stream reassembled, then envelope fallback (see below) | `ClaudeCodeProgressDigester` |
