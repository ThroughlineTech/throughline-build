...
- A `BypassPermissions` option (default true) decides whether to emit the agent's unattended-mode flag: `--dangerously-skip-permissions` ([ClaudeCodeAgent.cs:376-377](../../src/ThroughlineBuild.Workers.ClaudeCode/ClaudeCodeAgent.cs#L376-L377)), `--dangerously-bypass-approvals-and-sandbox` ([CodexAgent.cs:183-184](../../src/ThroughlineBuild.Workers.Codex/CodexAgent.cs#L183-L184)), `--yolo` ([GeminiAgent.cs:227-228](../../src/ThroughlineBuild.Workers.Gemini/GeminiAgent.cs#L227-L228)); Copilot always passes `-s --no-ask-user` and has **no** `BypassPermissions` option at all - `CopilotOptions` omits the field and the factory does not pass it ([Program.cs:792-798](../../src/ThroughlineBuild.Cli/Program.cs#L792-L798)). That asymmetry is called out again in "adding a worker agent".
...
Each phase is then constructed with `workerFactory.Create(EffectiveAgentFor(phase))` ([Program.cs:922](../../src/ThroughlineBuild.Cli/Program.cs#L922), [982](../../src/ThroughlineBuild.Cli/Program.cs#L982), [1109](../../src/ThroughlineBuild.Cli/Program.cs#L1109)). Selection precedence is computed by `EffectiveAgentFor` ([Program.cs:816-820](../../src/ThroughlineBuild.Cli/Program.cs#L816-L820)): a per-phase CLI flag (`--agent-plan`/`--agent-implement`/`--agent-review`) beats `--agent`, which beats the per-phase config entry (`AgentFor`, [Program.cs:811-812](../../src/ThroughlineBuild.Cli/Program.cs#L811-L812)), which falls back to `default_agent`. Per-phase agent picking ("Claude Code for planning, Codex for implementation, Gemini for review") is therefore implemented today, both via config (`[workers.phases]`) and via flags. An unknown agent name surfaces as a `ConfigException` from `WorkerAgentFactory.Create` ([WorkerAgentFactory.cs:28-36](../../src/ThroughlineBuild.Cli/WorkerAgentFactory.cs#L28-L36)).

The default agent is config-driven with no hardcoded vendor fallback in C# (`default_agent` is a required string). The product default and the live operator default now diverge: the embedded `build init` template still ships `default_agent = "claude-code"` ([config.toml.template:23](../../src/ThroughlineBuild.Commands/Templates/config.toml.template#L23)), but the checked-in operator config selects Codex - `.build/config.toml:25` sets `default_agent = "codex"` with the `[workers.codex]` block uncommented (sizes `small=gpt-5.4-mini`, `medium=gpt-5.4`, `large=gpt-5.5`). So a fresh `build init` generates a claude-code default; this repo's running config defaults to codex.

### Loose ends (worker layer)

- The factory body is a hardcoded `if`-chain over four known names ([Program.cs:776-805](../../src/ThroughlineBuild.Cli/Program.cs#L776-L805)) rather than a data-driven registry; adding a fifth agent requires editing this block in `Program.cs` (see "adding a worker agent" below).
- The `build new` draft-mode path resolves the implement-phase agent name but then constructs `ClaudeCodeAgent` unconditionally ([Program.cs:546-562](../../src/ThroughlineBuild.Cli/Program.cs#L546-L562)) - draft generation is effectively Claude-Code-only regardless of `default_agent`.
...
`LlmClientFactory.Create(LlmConfig, BuildSecrets, HttpClient)` ([src/ThroughlineBuild.Cli/LlmClientFactory.cs:8-29](../../src/ThroughlineBuild.Cli/LlmClientFactory.cs#L8-L29)) inspects the `[llm] default_model` prefix and **only** accepts `anthropic:` - any other prefix throws `ConfigException` with "only 'anthropic:' is supported". On the anthropic path it constructs `AnthropicClient` directly ([LlmClientFactory.cs:20](../../src/ThroughlineBuild.Cli/LlmClientFactory.cs#L20)). This is the proof that the model-client layer is single-vendor on the production path: the factory exists and reads a prefix, but it can return exactly one implementation. Note that `[llm] default_model` is now **deprecated for worker-model selection** - worker models come from `[workers.<agent>.sizes]`, and the live `.build/config.toml` comments `default_model` out entirely ([.build/config.toml:16-22](../../.build/config.toml#L16-L22)). `default_model` survives only as the input to this factory (i.e. for the judgment-slot path); with it commented out, `LlmClientFactory.Create` throws, which is exactly the degraded-mode trigger described next.

`ReasonTranslator` ([src/ThroughlineBuild.JudgmentSlots/ReasonTranslator.cs:25-31](../../src/ThroughlineBuild.JudgmentSlots/ReasonTranslator.cs#L25-L31)) is the only judgment-slot consumer - and the **only LLM consumer of any kind in the deterministic CLI**. It translates operator reason text to English when running `close` / `defer` / `reopen`, using the model `claude-haiku-4-5-20251001`, pinned to a `const` ([ReasonTranslator.cs:15](../../src/ThroughlineBuild.JudgmentSlots/ReasonTranslator.cs#L15)). It is wired in `Program.cs.WireUpConditionalCommands` ([src/ThroughlineBuild.Cli/Program.cs:1710-1738](../../src/ThroughlineBuild.Cli/Program.cs#L1710-L1738)), which now **degrades gracefully when no LLM client can be built** (TLB-371): if `LlmClientFactory.Create` throws `ConfigException` (no key, or `default_model` unset/commented), it logs a `WARNING: LLM unavailable (...)` line and substitutes an `EchoLlmClient` ([src/ThroughlineBuild.Cli/EchoLlmClient.cs](../../src/ThroughlineBuild.Cli/EchoLlmClient.cs)) that returns the operator's reason text verbatim, so the state transition still runs:

```csharp
try { llmClient = LlmClientFactory.Create(llmConfig, secrets, http); }   // Program.cs:1730 - anthropic only
catch (ConfigException ex) {
    Console.Error.WriteLine($"WARNING: LLM unavailable ({ex.Message}); recording reason verbatim without translation.");
    llmClient = new EchoLlmClient();                                      // Program.cs:1736 - verbatim fallback
}
var translator = new ReasonTranslator(llmClient);                        // Program.cs:1738
```
...
| `Workers.Codex/CodexAgent.cs` (+ `CodexJsonDtos.cs`, `CodexOptions.cs`, `CodexProgressDigester.cs`) | `codex exec [--dangerously-bypass-approvals-and-sandbox] --model <m> <prompt>`, brief as positional arg, `CODEX_API_KEY` / `OPENAI_API_KEY` removal, `openai:` prefix strip, `vendor: "openai"`, plain-stdout parse. |
...
5. Add `XOptions` with `ExecutablePath`, `MaxOutputTokens`, `Sizes` (the `WorkerSize -> model` map), and - if `X` has an unattended-mode flag - `BypassPermissions`. This last field is optional and asymmetric across the existing agents: Claude Code / Codex / Gemini all carry `BypassPermissions` and the factory forwards it, but Copilot does not - `CopilotOptions` omits the field entirely and the factory's `copilot` branch never sets it ([Program.cs:792-798](../../src/ThroughlineBuild.Cli/Program.cs#L792-L798)). Copilot's unattended mode (`-s --no-ask-user`) is unconditional, so there is no permission gate to toggle. If `X`'s CLI behaves like Copilot, follow that shape; otherwise mirror the other three.
...
7. Wire the factory: add a `capturedName == "x"` branch in the `Program.cs` factory body ([Program.cs:776-805](../../src/ThroughlineBuild.Cli/Program.cs#L776-L805)) and a project reference in `Cli.csproj`.
...
- **The worker factory body is a hardcoded `if`-chain** over four names ([Program.cs:776-805](../../src/ThroughlineBuild.Cli/Program.cs#L776-L805)) rather than a data-driven registry. The `WorkerAgentFactory` indirection exists, but the construction switch still lives in `Program.cs`.
