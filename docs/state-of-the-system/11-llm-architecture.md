...
Last refreshed: 2026-06-14 (`heartbeat-stage-07-cutover`, Stage 07; documents the `ClaudeCodePreflight` capability gate, on top of the Stage 05-06 process-hardening + completion-redesign code surface. The transport default stays `print` until the post-dogfood cutover flip; interactive-hook is opt-in. Dogfood validation is operator-run and pending; see [docs/heartbeat/evidence/stage-07-dogfood.md](../heartbeat/evidence/stage-07-dogfood.md))
...
|---|---|---|---|---|---|
| `ClaudeCodeAgent` | `claude-code` / `anthropic` | Default `print` transport (`claude --print --verbose --output-format stream-json [...]`, brief on stdin); opt-in `interactive-hook` (interactive Claude under a terminal host, no `--print`) is the Stage 07 cutover target | removes `ANTHROPIC_API_KEY`, sets `CLAUDE_CODE_MAX_OUTPUT_TOKENS` | print: NDJSON stream reassembled, then envelope fallback (see below); interactive: completion synthesized from the persisted transcript | `ClaudeCodeProgressDigester` |
