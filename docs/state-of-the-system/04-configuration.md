...
Last refreshed: 2026-06-14 (`heartbeat-stage-07-cutover`, Stage 07: added the Claude transport capability preflight (`build setup` + pre-phase + transport-entry); the `[workers.claude-code].transport` default stays `print` until the post-dogfood cutover flip, with interactive-hook opt-in)
...
`transport` is parsed for any Claude-family agent name (not gemini/codex/copilot) and mapped by `WorkerAgentBuilder` into `ClaudeCodeOptions.Transport`. Omission resolves to `print` (the config loader's omitted-value default at [src/ThroughlineBuild.Cli/Config.cs](../../src/ThroughlineBuild.Cli/Config.cs)) until the Stage 07 cutover flips that default to `interactive-hook` after operator dogfood; until then the generated `build init` template leaves `transport` commented and interactive-hook is opt-in (`transport = "interactive-hook"`). The `ClaudeCodeOptions`/`AgentConfig` type defaults also stay `Print`. The interactive host is implemented on Windows (ConPTY) and Unix (PTY); non-debug runs remove their private per-invocation run directory, while debug runs retain settings and completion evidence under the configured capture directory ([`ClaudeCodeInteractiveTransport.ExecuteAsync`](../../src/ThroughlineBuild.Workers.ClaudeCode/ClaudeCodeInteractiveTransport.cs)).
