...
Last refreshed: 2026-06-14 (`heartbeat-stage-07-cutover`, Stage 07 cutover: `[workers.claude-code].transport` now defaults to `interactive-hook`, with a `build setup` / pre-phase capability preflight)
...
**There is now exactly one template.** The old hand-edit reference `.build/config.toml.example` has been deleted; the embedded `build init` template [src/ThroughlineBuild.Commands/Templates/config.toml.template](../../src/ThroughlineBuild.Commands/Templates/config.toml.template) is the single documented shape, using `REQUIRED_*` placeholders for the four Plane fields. Template and the live checked-in operator config [.build/config.toml](../../.build/config.toml) both set `default_agent = "claude-code"` with an active `[workers.codex]` block as the alternate - the template-vs-live `default_agent` drift flagged in the previous refresh is gone. The template now ships **empty review/ship checks and a blank `[project]` toolchain on purpose** (commit 187d1ca "make build toolchain no-op by default"): checks and toolchain fields are derived from the op-doc at `build scaffold` time, so a project with no build/test step passes the gate instead of hard-failing on a phantom `dotnet build`; commented per-stack examples remain for hand configuration.
...
| `transport` | no | `interactive-hook` | Claude Code only. Accepted values are `print` and `interactive-hook`; unknown Claude values hard-fail config loading and name both supported values. On any other agent block, `transport` emits the standard unknown-key warning and is ignored. As of the Stage 07 cutover the omitted-value default is `interactive-hook`; `print` is the one-line rollback. `interactive-hook` launches a fresh interactive Claude session under a terminal host (ConPTY on Windows, PTY on Unix; no `--print`), synthesizes the completion from claude's persisted transcript, parses the full final assistant response, and never falls back to `--print`. `ClaudeCodePreflight` gates it (claude runnable, version >= 2.1.177, platform supported) before a worker-spawning phase and in `build setup` ([`ClaudeCodeInteractiveTransport.ExecuteAsync`](../../src/ThroughlineBuild.Workers.ClaudeCode/ClaudeCodeInteractiveTransport.cs), [src/ThroughlineBuild.Workers.ClaudeCode/ClaudeCodePreflight.cs](../../src/ThroughlineBuild.Workers.ClaudeCode/ClaudeCodePreflight.cs)). |
...
`transport` is read only from `[workers.claude-code]` and mapped by `WorkerAgentBuilder` into `ClaudeCodeOptions.Transport`. As of the Stage 07 cutover, omission resolves to `interactive-hook` (the config loader's omitted-value default at [src/ThroughlineBuild.Cli/Config.cs](../../src/ThroughlineBuild.Cli/Config.cs)) and the generated `build init` template writes `transport = "interactive-hook"` explicitly; set `transport = "print"` for the one-line rollback to the legacy headless path. The `ClaudeCodeOptions`/`AgentConfig` type defaults stay `Print` for direct construction, so the loaded-config default is the load-bearing one. The interactive host is implemented on Windows (ConPTY) and Unix (PTY); non-debug runs remove their private per-invocation run directory, while debug runs retain settings and completion evidence under the configured capture directory ([`ClaudeCodeInteractiveTransport.ExecuteAsync`](../../src/ThroughlineBuild.Workers.ClaudeCode/ClaudeCodeInteractiveTransport.cs)).
