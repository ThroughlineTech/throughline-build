...
| 2026-06-14 | `heartbeat-stage-07-cutover` (Stage 07 cutover) | targeted | Update-as-you-go for the Stage 07 Claude-transport cutover. Flipped the loaded-config default for `[workers.claude-code].transport` from `print` to `interactive-hook` (the config loader's omitted-value default + the generated `build init` template; the `ClaudeCodeOptions`/`AgentConfig` type defaults stay `Print` for direct construction, and `print` is the one-line rollback). Added `ClaudeCodePreflight` + `ClaudeTransportPreflight`: a capability gate run by `build setup` and before any worker-spawning phase (claude runnable, `claude --version` >= 2.1.177, platform supported), failing clearly and never silently falling back to print. Updated [04-configuration.md](04-configuration.md) (transport default, omission prose, `build setup` note), [11-llm-architecture.md](11-llm-architecture.md) (header + default-transport prose + agent table cell), and the `build setup` entries in [01-inventory.md](01-inventory.md) and [06-public-surfaces.md](06-public-surfaces.md). Dogfood validation of the cutover is operator-run and pending ([docs/heartbeat/evidence/stage-07-dogfood.md](../heartbeat/evidence/stage-07-dogfood.md)). No numbered docs added or removed. |
