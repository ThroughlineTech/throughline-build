...
src/ThroughlineBuild.ClaudeCode/          - reusable public Claude Code facade
...
| `01-inventory.md` | The 38 CLI action verbs, 20 src projects (1 entry + 19 libraries), 2 AOT tools (`token-audit`, `analyze-event-log`), scripts and CI - what each is, what it reads/writes, status. |
...
| `06-public-surfaces.md` | CLI surface (including the versioned `--json` envelope and tiered help), the reusable Claude Code facade, library-level public types, and stability call-outs. |
...
`src/ThroughlineBuild.ClaudeCode/`,
...
- 2026-08-11 - full code-true refresh against HEAD `758ad56a`. No documents were added or removed. The set now records 38 verbs and 36 help entries; cursor-paginated Plane comment reads; ticket attachment list/download with membership, redirect-auth, JSON, and atomic-file boundaries; prefix-aware candidate lease matching; tracked dogfood setup/build/test/install commands including the Windows PowerShell orphan-testhost guard and frozen restore; the ignored project-less tools lockfile; and Claude trust-seeder test isolation. All 12 breadcrumb pairs were reverified and the five affected `AGENTS.md` pointers were corrected; their `CLAUDE.md` shims remained valid.
...
- The numbered state-of-the-system set is maintained as the as-built current reference. The former standalone architecture document is a retirement pointer. Source and generated help remain authoritative; any disagreement is treated as a documentation bug.
...
**2026-08-11 refresh judgment calls.**

- **No new documents.** Attachments are a ticketing/public-contract/state/failure concern and fit the existing ten-question structure; splitting them into a feature document would weaken the question-to-document map.
- **Defaults are separated from active repository choices.** Config-loader/template omission still selects Claude `interactive-hook`, while this tracked repository config explicitly selects `print`; the docs state both rather than calling either one the universal current transport.
- **The project-less tool lockfile is not a solution lock.** Locked restore remains true for all 20 source and 20 test projects. `src/tools/packages.lock.json` is generated RID/SDK debris and is now ignored, so it is explicitly excluded from the claim.
- **Breadcrumb edits were in scope.** The prompt itself requires correcting nested breadcrumb drift. Five affected `AGENTS.md` files were updated; every sibling `CLAUDE.md` remained the required pointer/import shim.
...
| 2026-06-13 | `cf3fe2b` on `heartbeat-stage-06-process-hardening` | targeted | Recorded the real macOS 26.4 arm64 validation of `cf3fe2b` in [11-llm-architecture.md](11-llm-architecture.md) (its Unix runtime stamp moved from "pending CI" to "validated on macOS arm64, linux-x64 pending"): both `UnixProcessTreeCleanupTests` cases passed against real `sh -> sleep` trees with zero leftover processes, every `libc` symbol resolved, the full 2441-test solution was green, and the osx-arm64 native AOT publish was warning-free and ran. Also applied a live-test launcher fix (route `LiveSentinel` through `InteractiveClaudeProcessLauncherFactory.Create()` instead of a hardcoded Windows host, and move its `--debug` capture outside the throwaway worktree) and noted the open interactive-hook completion item (Stage 03/04 layer, not the Stage 06 PTY layer). The documented production code surface is unchanged since `682f333`, so `11`'s code stamp stays `682f333`. |
| 2026-06-13 | `3088bb9` on `heartbeat-stage-06-process-hardening` | targeted | Added a "Known live-claude blocker" caveat to the interactive-transport entry in [11-llm-architecture.md](11-llm-architecture.md): a controlled macOS run (claude 2.1.170) pinned that the per-turn `--settings` Stop hook does not fire in claude's interactive session (only in `--print`), so the trusted-completion round-trip times out and the interactive transport's completion detection must move off the per-turn Stop hook before cutover - a completion-contract issue, not the Stage 06 process-hosting layer. Code surface unchanged; only the caveat + a pointer to the Stage 06 evidence doc were added. |
| 2026-06-13 | `a84da6e` on `heartbeat-stage-06-process-hardening` | targeted | Recorded the Linux/glibc validation and the confirmed completion fix-direction in [11-llm-architecture.md](11-llm-architecture.md). The Unix PTY host's runtime stamp moved from "macOS only, linux pending" to "validated on macOS 26.4 arm64 AND Ubuntu 24.04 x86_64 / glibc 2.39" (both `UnixProcessTreeCleanupTests` pass, all `libc` symbols resolve, full solution green, linux-x64 + osx-arm64 AOT clean - the Linux run was pinned to `3088bb9`). The "Known live-claude blocker" caveat was sharpened: a follow-up macOS experiment confirmed the Stop hook fires on interactive `/exit` (not per-turn) with the right `last_assistant_message`, so the fix keeps the `completion.json` contract (detect turn-done -> `/exit` to the PTY) but must pre-seed `~/.claude.json` workspace trust for fresh worktrees. Code surface still unchanged since `682f333`, so `11`'s code stamp stays `682f333`. Rewrote the interactive-transport entry in [11-llm-architecture.md](11-llm-architecture.md) to record Stage 05 observability (the isolated `ClaudePersistedTranscriptReader`: full multi-message transcript, model identity, token/cache usage marked unavailable not invented, per-turn `context_turns`, provider-error text, redacted debug side channel; optional telemetry failure never fails a valid result) and Stage 06 process/terminal hardening (the platform terminal-host abstraction + factory; Windows ConPTY with a mandatory kill-on-close job object; the implemented Unix `posix_openpt`/`posix_spawnp` PTY host with `POSIX_SPAWN_SETSID` process-group containment, whose runtime behavior is exercised by the Linux/macOS CI tree-cleanup tests, pending this branch's first CI run, not on the Windows dev host; the bounded graceful-then-forced `ProcessShutdownSequence`; per-run `ClaudeRunLease` + lock-based `ClaudeRunDirectorySweeper`; the per-worktree `InteractiveClaudeWorktreeLock`). Only `11` touched; the generic print-path process-tree-kill prose in `03`/`09` was re-checked and remains accurate. |
| 2026-06-14 | `87266fc` on `heartbeat-interactive-completion-redesign` | targeted | Update-as-you-go refresh for the interactive-completion redesign. Rewrote the interactive-transport completion mechanism in [11-llm-architecture.md](11-llm-architecture.md): completion is now synthesized from `claude`'s persisted transcript (turn-detect via `TranscriptTurnSignal`/`ClaudeTranscriptLocator` for `stop_reason == end_turn` -> synthesize `ClaudeCompletionRecord` -> best-effort `/exit` nudge -> terminate the process tree -> parse `WORKER_RESULT` + telemetry), with the Stop-hook `completion.json` demoted to a best-effort fast-path because `claude` 2.1.170+ does not fire the per-turn `--settings` Stop hook in interactive mode (nor on Linux session exit); also recorded trust pre-seeding in `~/.claude.json` and `ClaudeRealPath.Resolve` path canonicalization. Validated live GREEN on macOS 26.4 arm64 (3/3) and Ubuntu 24.04 x86_64 / glibc 2.39 (PASS) against `claude` 2.1.177; the Linux synthesized `completion.json` carried `stop_hook_active: false`. Windows 11 live also passes (claude 2.1.177, dev host) - all three supported platforms green. Code stamp on `11` left at `682f333` is superseded by the branch note (this work lands off the Stage 06 tip, not yet on `main`). Documented production code surface advanced by this branch; the doc records the new mechanism. |
| 2026-06-14 | `heartbeat-stage-07-cutover` (Stage 07) | targeted | Update-as-you-go for Stage 07. Added `ClaudeCodePreflight` + `ClaudeTransportPreflight`: a capability gate (claude runnable with `--version` exit 0, version >= 2.1.177, platform supported) run by `build setup`, before the worker-spawning phase verbs, AND at the interactive transport entry (`ExecuteAsync`) so every worker-spawning path is covered (draft, investigate-plan, scaffold, batch); it fails clearly and never silently falls back to print. `transport` is now honored on any Claude-family agent name (not just `claude-code`), so the print rollback works on custom-named blocks. The default transport stays `print`: the cutover flip to `interactive-hook` is deferred to after operator dogfood + the pre-cutover checkpoint (interactive-hook is opt-in until then), per the prompt's required sequence. Updated [04-configuration.md](04-configuration.md) (transport row default + omission prose + `build setup` note), [11-llm-architecture.md](11-llm-architecture.md) (header + default-transport prose + agent table cell), and the `build setup` entries in [01-inventory.md](01-inventory.md) and [06-public-surfaces.md](06-public-surfaces.md). Dogfood validation was operator-run and pending at that checkpoint; its former `docs/heartbeat/evidence/stage-07-dogfood.md` report was later removed with the heartbeat archive. No numbered docs added or removed. |
| 2026-06-15 | `heartbeat-stage-07-finish-cutover` (Stage 07 cutover) | targeted | Landed the Stage 07 default-transport cutover after the npt5 operator dogfood validated a clean interactive chain (merged to `main`). Flipped the config-loader omitted-value default in [src/ThroughlineBuild.Cli/Config.cs](../../src/ThroughlineBuild.Cli/Config.cs) from `Print` to `InteractiveHook` and kept the generated `build init` template's explicit `transport = "interactive-hook"`; `print` is now the documented rollback. Updated [04-configuration.md](04-configuration.md) (header + transport row default + omission prose) and [11-llm-architecture.md](11-llm-architecture.md) (header + agent table cell + default-transport prose), plus the breadcrumb [src/ThroughlineBuild.Workers.ClaudeCode/AGENTS.md](../../src/ThroughlineBuild.Workers.ClaudeCode/AGENTS.md), the `ClaudeCodeOptions.Transport` comment, the userguide template + its generated `docs/throughline_build_userguide.md` copy, `docs/heartbeat/HANDOFF.md`, and the Stage 07 evidence doc. Flipped + renamed the two `ConfigLoaderTests` assertions that pinned the pre-cutover `Print` default. The `ClaudeCodeOptions`/`AgentConfig` type-level defaults stay `Print` (direct construction only). No numbered docs added or removed. |
...
| 2026-08-11 | `758ad56a` on `run/backlog` | full | Full code-true refresh. Updated all 12 numbered documents plus this prompt record in place; no document was added or removed. Reverified all 12 breadcrumb pairs and corrected five affected `AGENTS.md` files; all `CLAUDE.md` shims remained valid. Absorbed 38 verbs/36 help entries, cursor-paginated comments, ticket attachment inventory/download and its AOT JSON/contracts/storage boundary, prefix-aware candidate lease matching, tracked PowerShell/setup/build/test/frozen-install dogfood configuration, ignored `src/tools/packages.lock.json`, and Claude trust-seeder test isolation. The solution remains 20 source and 20 mirrored test projects, with 2,852 test declarations across 266 tracked C# test files. |
| 2026-08-12 | `758ad56a` on `main` | targeted | Consolidated the operator and new-repository setup guides after clean-clone dogfood on secure-bike-parking and sqzarr. Retired the drifted standalone architecture document to a compatibility pointer and made this numbered set the explicit current architecture authority. Updated `00`, `01`, `02`, `05`, and `10` only where authority or obsolete-link wording changed; no implementation claim or status tag changed. |
