...
| `01-inventory.md` | The 20 CLI verbs (incl. `op-doc`, `models`, `setup`) plus `help` and `-V`/`--version`, the tiered help registry, 19 src projects (1 entry + 18 libraries), 2 AOT tools (`token-audit`, `analyze-event-log`), scripts and CI - what each is, what it reads/writes, status. |
...
- 2026-06-08 - full code-true refresh against HEAD `de1909e` (173 commits past `420d9c4`). No documents added or removed; the same 13-file set was rewritten in place and all 9 breadcrumb `AGENTS.md` files re-verified and corrected. Largest single window since the 2026-05-30 baseline pass. The major new subsystem is the **op-30 deterministic verification gate** (`GatePhase` between implement and review, the worker-emitted `CompletionClaim`/`COMPLETION_CLAIM` contract, the `[[review.checks]]` capability map with Gating/Advisory roles, smoke signals, consumes/provides preflight, gate-check reuse into review, and structured gate-failure routing to rework), which flipped `Phase` from 10 to 11 (added `Gate`) and `EventKind` from 13 to 14 (added `CostLedger`, TLB-510). Also absorbed: **op-32 batch-implement** (`build chain --batch-implement`, `BatchCommitVerifier`, partial-failure recovery, combined-stack review, localized/cross-ticket rework routing, the `[batch]` size caps); **op-33 Codex model tiers** (`{model, effort}` `ModelTier` sizes, `build models refresh` + the `codex debug models` probe, reasoning-effort argv/telemetry); **op-34 connected onboarding** (interactive create-or-pick `build init`, the new `build setup` provisioner, `IProjectResolver`/`IProjectDiscovery`/`ITicketingProvisioner`/`ITicketingConnectivity` on `PlaneTicketingClient`); the **tiered help registry** (`Help/` with `HelpRegistryFactory` + `Tier0`/`Tier1`/topic renderers) that retired `CliUsage.cs` to test-only; the new `op-doc spec|new` verb and `-V`/`--version` flag; recursive chain integration branches (TLB-475/492/494) replacing the one-level parent model; and a batch of structured-failure-salvage and git-hardening fixes (TLB-471/472/476, UTF-8 pinning, malformed-JSONL tolerance, sibling/negative-id ordering). The verbatim prompt above was unchanged from the prior run.
...
**2026-06-08 refresh judgment calls.**

- **No new documents.** The 173-commit window was the largest since the 2026-05-30 baseline (a verification gate, batch-implement, model tiers, connected onboarding, a help system, two new verbs), but every delta maps onto the existing ten-question structure. The gate folded into `06`/`07`/`09`/`10`/`11`; batch-implement into `09`/`10` and the chain verb in `01`; model tiers into `04`/`11`/`03`; onboarding into `01`/`02`/`03`/`07`/`08`; the help registry into `01`/`06`. Splitting out a new doc for the gate was considered and rejected - it is a phase in the existing state machine, not a parallel subsystem, so it reads best inside `10`. The clean question-to-doc mapping was kept.
- **Parallel doc-editing dispatch.** Because the window touched all twelve numbered docs and Program.cs/ChainPhase.cs/Config.cs had all grown by hundreds of lines (so nearly every cite drifted), the numbered docs were refreshed by parallel per-doc agents working from one shared canonical-facts brief, then the cross-cutting `00-index.md`, `PROMPT.md`, and the nine breadcrumbs were reconciled by hand. Each agent re-opened the live source to confirm every cite it touched; this caught several drifts the brief did not anticipate.
- **`default_agent` split resolved.** The prior refresh flagged a template-vs-checked-in default-agent split (template `claude-code`, checked-in `.build/config.toml` `codex`). At HEAD both default to `claude-code` (commits `e413c15`/`8d0405c` reverted the checked-in config back to claude). The loose end is closed; `00`/`03`/`04` were corrected to state a single default. There is still no hardcoded vendor default in C#.
- **`CliUsage.cs` is now dead in production.** Help moved to the `Help/` registry (`HelpRegistryFactory` + `Tier0`/`Tier1`/topic renderers); `Program.cs` no longer references `CliUsage.UsageText`, which survives only as a test fixture. The docs now treat the registry as the live help surface and tag `CliUsage.cs` Legacy; the inventory and the `Cli` breadcrumb were corrected (the breadcrumb had drifted to "~1774 lines, 18 names, usage text in CliUsage.cs").
- **Parent chain is recursive now, not one level.** The 2026-06-02 docs described a one-level parent chain guarded by a `ParentHasGrandchildren` grandchild-stop. TLB-475/492/494 made it recurse to `--max-depth` over accumulating integration branches; `ParentHasGrandchildren` is now effectively unreachable and integration/leaf branches are retained at chain end for resume. `08`/`09`/`10` were rewritten accordingly; the code won over the prior prose.
- **Gate hooks and the cost-ledger are the new Aspirational items.** `CompletionClaim.RedGreenKind`/`Tier`/`RoutingKey` are declared but read by nobody, and the `CostLedger` event is emitted but has no in-`src` consumer/renderer. Both are documented as built-but-unconsumed rather than omitted. The `ModelClient` layer (`IModelClient`/`AnthropicModelClient`/`ModelClientLlmAdapter`) remains built-but-unwired, unchanged.
- **`[batch]` caps vs the review-size threshold.** `[batch]` config carries only `max_tickets`/`max_size_score`/`max_description_bytes`; the batch combined-review second-pass threshold (default 5) is a `BuildOptions` field not wired to config. The docs state this distinction so a reader does not look for a `batch_review_size_threshold` key that does not exist.
- **Verbatim prompt unchanged** from the prior run (same CONFIG + PROMPT body, same nine breadcrumb locations).
...
| 2026-06-08 | `de1909e` on `main` | Full code-true refresh; 173 commits past `420d9c4` (largest window since the 2026-05-30 baseline). All 13 files rewritten in place (no docs added/removed) and all 9 breadcrumb `AGENTS.md` files re-verified and corrected. Numbered docs refreshed by parallel per-doc agents from one shared canonical-facts brief; `00`/`PROMPT`/breadcrumbs reconciled by hand. Major code deltas absorbed: **op-30 verification gate** - `GatePhase` between implement and review, worker `CompletionClaim`/`COMPLETION_CLAIM` contract (`CompletionClaimParser`), `[[review.checks]]` capability map with `CheckRole` Gating/Advisory, `SmokeCollector`/`SmokeSignal`, consumes/provides preflight, `PreComputedChecksRunner` check-reuse, gate-failure-to-rework (`ReviewFeedback.GateFailedChecks`), `EmitCostLedgerAsync` - flipping `Phase` 10 -> 11 (`Gate`) and `EventKind` 13 -> 14 (`CostLedger`, TLB-510); **op-32 batch-implement** (`build chain --batch-implement`, `BatchCommitVerifier`, `BatchTicketResult`/`WorkerResult.Tickets`, `ChainOutcome.BatchImplemented`, partial-failure recovery, combined-stack review, localized/cross-ticket rework, `[batch]` caps); **op-33 Codex model tiers** (`ModelTier` `{model, effort}` sizes, `build models refresh` + `CodexModelProbe`/`CodexTierMapper`/`CodexSizesBlock*`, reasoning-effort argv + telemetry); **op-34 connected onboarding** (interactive create-or-pick `build init`, new `build setup`, `CredsFileParser`/`WelcomeCommit`/`LocalRepoSetup`, `IProjectResolver`/`IProjectDiscovery`/`ITicketingProvisioner`/`ITicketingConnectivity` on `PlaneTicketingClient`, `WorkspaceSchema`); the **tiered help registry** (`Help/` `HelpRegistryFactory` + `Tier0`/`Tier1`/`HelpTopicRegistry`, retiring `CliUsage.cs` to test-only); new verbs `op-doc spec|new` and the `-V`/`--version` flag (`BuildVersion`); **recursive chain integration branches** (TLB-475/492/494) with retained branches for resume (the one-level `ParentHasGrandchildren` guard is now unreachable); `ChainPhaseComposition` wiring seam + `ChainExitCodeMapper`; and structured-failure-salvage/git-hardening (TLB-471/472/476, UTF-8 stdout pin TLB-439, malformed-JSONL tolerance, sibling/negative-id ordering TLB-511/496). Project count stayed 19 (1 entry + 18 libs), all net10.0. `default_agent` template-vs-checked-in split resolved (both `claude-code`). Verbatim prompt unchanged. Note: `Program.cs` (~1654 -> ~2209), `ChainPhase.cs`, and `Config.cs` (~470 -> ~869) all grew by hundreds of lines, so the large majority of cites were re-derived against HEAD. |
