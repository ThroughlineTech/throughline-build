...
Three pre-passes run before verb dispatch: bare bool flags `--debug`, `--quiet`, `--summary-json`, `--error-location`, `--no-auto-resolve`, `--no-auto-merge`, `--no-push`, `--continue-past-failure`, `--from-brief`, `--skip-baseline` are stripped first ([src/ThroughlineBuild.Cli/Program.cs:34-69](../../src/ThroughlineBuild.Cli/Program.cs#L34-L69)); then `--agent` / `--agent-plan` / `--agent-implement` / `--agent-review` pairs are extracted ([src/ThroughlineBuild.Cli/CliArgParser.cs:24-65](../../src/ThroughlineBuild.Cli/CliArgParser.cs#L24-L65)); then ticket IDs for phase verbs are extracted ([src/ThroughlineBuild.Cli/CliArgParser.cs:94-118](../../src/ThroughlineBuild.Cli/CliArgParser.cs#L94-L118)).

Seventeen verbs are reachable: `init`, `settarget`, `user-guide`, `plan`, `implement`, `review`, `ship`, `chain`, `rework`, `decompose`, `new`, `scaffold`, `list`, `amend`, `close`, `defer`, `reopen`, and `--help`/`help`. Any other token returns exit 2 (unknown subcommand). `init`, `settarget`, and `user-guide` all dispatch before config load (`init`/`settarget` edit the config itself; `user-guide` writes a doc and needs no config).

### `build init [--force --print-template --plane-url --workspace --project-id --token|--token-env]` - Functional
[src/ThroughlineBuild.Cli/Program.cs:138-153](../../src/ThroughlineBuild.Cli/Program.cs#L138-L153), implemented in [src/ThroughlineBuild.Cli/InitCommand.cs](../../src/ThroughlineBuild.Cli/InitCommand.cs). Bootstraps `.build/config.toml` from the embedded template before any config is loaded (it runs ahead of config discovery). Not an `ITicketCommand`. When stdin is a TTY (not redirected), `init` interactively prompts for any missing `REQUIRED_*` values - plane base URL, workspace, project id, API token - before applying flags (TLB-370, [src/ThroughlineBuild.Cli/InitCommand.cs:37-38](../../src/ThroughlineBuild.Cli/InitCommand.cs#L37-L38)); explicit flag values still win over prompted ones. The success message points the operator at `build user-guide`.
...
[src/ThroughlineBuild.Cli/Program.cs:167-172](../../src/ThroughlineBuild.Cli/Program.cs#L167-L172), implemented in [src/ThroughlineBuild.Cli/UserGuideCommand.cs](../../src/ThroughlineBuild.Cli/UserGuideCommand.cs) (TLB-322). Writes the embedded operator guide to `docs/throughline_build_userguide.md`. Like `init`/`settarget`, it runs ahead of config discovery and needs no `.build/config.toml`. Not an `ITicketCommand`. The guide body is loaded from an embedded resource by `UserGuideLoader` ([src/ThroughlineBuild.Cli/UserGuideCommand.cs:21](../../src/ThroughlineBuild.Cli/UserGuideCommand.cs#L21)).
...
| `ThroughlineBuild.Phases` | Functional | Phase classes: `PlanPhase`, `ImplementPhase`, `ReviewPhase`, `ShipPhase`, `ChainPhase`, `ReworkPhase`, `DecomposePhase`, `NewPhase`, `DraftPhase`. Plus multi-ticket orchestration: `ParallelDispatcher` (concurrency now pinned to 1 - effectively serial), `TicketGraph` (+`TopologicalSorter`, Kahn-BFS levels), `AncestorSkipFilter`, and `EarlyExitManifest`. Hygiene/marker helpers: `WorkingTreeHygieneGate` (internal static; conflicted-path + dangling-stash checks run before implement, at chain preflight, and before ship) and `CommentMarkers` (`LatestValue` selects the freshest marker by timestamp, not list order). |
...
- **User-guide template is now surfaced (TLB-322, was TLB-320).** The embedded operator guide is emitted by the `build user-guide` verb ([src/ThroughlineBuild.Cli/UserGuideCommand.cs](../../src/ThroughlineBuild.Cli/UserGuideCommand.cs), dispatch [src/ThroughlineBuild.Cli/Program.cs:167-172](../../src/ThroughlineBuild.Cli/Program.cs#L167-L172)), which loads it from an embedded resource via `UserGuideLoader` and writes `docs/throughline_build_userguide.md`. The prior loose-end (template compiled in but reachable from no verb) is resolved.
