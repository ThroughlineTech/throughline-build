...
Last refreshed: 2026-07-26 (TLB-568)
...
`Program.cs` is a three-line process entry point that delegates to `CliApplication.RunAsync`. `CliVerbRegistryFactory` declares all 26 action verbs and records whether each runs before configuration bootstrap; `CliApplication` resolves the requested entry before executing either the pre-config band or the configured band. `CliBootstrap` resolves the main worktree, loads config and secrets, and creates one shared `HttpClient` and `PlaneTicketingClient` in an immutable `CliContext`. Help remains a separate registry: `HelpRegistryFactory.Build` declares one `HelpModel` per documented command (24 entries, grouped Pipeline / Work items / Configure); `Tier0Renderer.Render` produces the grouped top-level menu, `Tier1Renderer.Render` produces per-command help, and `HelpTopicRegistry.Build` declares the `config`, `digest`, `exit-codes`, and `summary` reference topics.

All argument pre-passes now live in `CliArgParser`: `ExtractBoolFlags` strips the global bare flags, `ExtractAgentFlags` pulls the `--agent` / `--agent-plan` / `--agent-implement` / `--agent-review` pairs, `ExtractChainTraversalFlags` pulls `--dry-run` / `--max-depth`, `ExtractBatchImplementFlag` pulls `--batch-implement`, and `ExtractTicketIds` extracts phase ticket IDs. Focused unit tests cover each pre-pass and registry tests pin the five pre-config verbs so their ordering cannot silently regress.
