...
Dispatched at [Program.cs:563-600](../../src/ThroughlineBuild.Cli/Program.cs#L563-L600), implemented by `SetupCommand.ExecuteAsync` ([SetupCommand.cs:33](../../src/ThroughlineBuild.Cli/SetupCommand.cs#L33)). Makes a freshly init'd project workflow-ready: (1) local repo - `git init` if needed plus a language-neutral `.gitignore` block via `GitignoreManager`/`FileSystemLocalRepoOps` ([LocalRepoSetup.cs:13](../../src/ThroughlineBuild.Cli/LocalRepoSetup.cs#L13)), then the welcome commit; (2) Plane - creates any missing states and labels against `WorkspaceSchema` (the canonical 7 states in `WorkspaceSchema.States` and 9 labels in `WorkspaceSchema.Labels`, [WorkspaceSchema.cs:23](../../src/ThroughlineBuild.Contracts/WorkspaceSchema.cs#L23), [WorkspaceSchema.cs:39](../../src/ThroughlineBuild.Contracts/WorkspaceSchema.cs#L39)) through the `ITicketingProvisioner` face of `PlaneTicketingClient`. Idempotent. `--check` mutates nothing and exits 1 when any local or Plane gap remains, else 0. After Plane provisioning it also runs a Claude transport capability preflight (`ClaudeTransportPreflight.ReportAsync` -> `ClaudeCodePreflight`): for each configured Claude agent it reports the transport and whether this host can run it (executable, `claude --version` >= 2.1.177 for `interactive-hook`, platform), returning non-zero when an `interactive-hook` agent is unsupported (Stage 07 cutover).
