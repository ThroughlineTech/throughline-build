...
### `build amend <ticket-id> [--title "..."] [--priority urgent|high|medium|low|none] [--type <name>] [--label-add <name>]... [--label-remove <name>]... [--parent <id>] [--size S|M|L] [--note "..."] [--description <path|->]` - Functional
Implemented by `AmendCommand.ExecuteAsync` ([AmendCommand.cs:17](../../src/ThroughlineBuild.Commands/AmendCommand.cs#L17)). It resolves and validates all file/stdin payloads, parent, issue type, and labels before the first mutation, then applies title, priority, type, labels/size, parent, note, and description sequentially. The write set is not transactional: a later failure can leave earlier updates committed. `--description` replaces the complete Plane description through `UpdateDescriptionAsync`. The acceptance-criteria option was removed in TLB-650: it aliased whole-description replacement rather than isolating a subsection, so no amend option now targets an acceptance-criteria section. Done and Cancelled tickets are refused.
