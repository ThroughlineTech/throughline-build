...
`Program.cs` is a three-line process entry point that delegates to `CliApplication.RunAsync`. `CliVerbRegistryFactory.Verbs` declares all 36 action verbs and records whether each runs before configuration bootstrap ([CliVerbRegistryFactory.cs:7](../../src/ThroughlineBuild.Cli/CliVerbRegistryFactory.cs#L7)); `CliApplication` resolves the requested entry before executing either the pre-config band or the configured band. `CliBootstrap` resolves the repository layout, loads config and secrets, and creates one shared `HttpClient` and `PlaneTicketingClient` in an immutable `CliContext`. Help remains a separate registry: `HelpRegistryFactory.Build` declares 34 entries in four groups (Pipeline, Bring your own conductor, Work items, Configure) ([HelpRegistryFactory.cs:25](../../src/ThroughlineBuild.Cli/Help/HelpRegistryFactory.cs#L25)); `Tier0Renderer.Render` produces the grouped top-level menu, `Tier1Renderer.Render` produces per-command help, and `HelpTopicRegistry.Build` declares the reference topics.
...
Thirty-six action verbs are reachable: `init`, `install`, `settarget`, `user-guide`, `op-doc`, `models`, `sop`, `conductor`, `profile`, `sweep`, `candidate`, `worker`, `worktree`, `gate`, `waves`, `list`, `get`, `comments`, `comment`, `evidence`, `transition`, `relate`, `setup`, `amend`, `close`, `defer`, `reopen`, `new`, `scaffold`, `rework`, `decompose`, `plan`, `implement`, `review`, `ship`, and `chain`, plus help and version meta-surfaces. `CliApplication.RunAsync` performs dispatch ([CliApplication.cs:34](../../src/ThroughlineBuild.Cli/CliApplication.cs#L34)). `init`, `install`, `settarget`, `user-guide`, `op-doc`, `models`, `sop`, `conductor`, and `profile` dispatch before full config load.
...
Dispatched by `CliApplication` ([CliApplication.cs:1162](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1162)), implemented by `SetupCommand.ExecuteAsync` ([SetupCommand.cs:40](../../src/ThroughlineBuild.Cli/SetupCommand.cs#L40)). Makes a freshly init'd project workflow-ready: (1) local repo - `git init` if needed plus a language-neutral `.gitignore` block via `GitignoreManager`/`FileSystemLocalRepoOps`, then the welcome commit; (2) Plane - creates any missing states and labels against `WorkspaceSchema` through the `ITicketingProvisioner` face of `PlaneTicketingClient`. Idempotent. `--check` mutates nothing and exits 1 when any local or Plane gap remains, else 0. `--write-token-file` persists the already-resolved Plane token to a named file and writes only its path into config; it cannot be combined with `--check` ([CliApplication.cs:1163](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1163), [TokenFileInstaller.cs:14](../../src/ThroughlineBuild.Cli/TokenFileInstaller.cs#L14)). After Plane provisioning setup also runs Claude transport capability preflight.
...
`CandidateStatusCommand.ExecuteAsync` fingerprints a current worktree without mutation ([CandidateStatusCommand.cs:15](../../src/ThroughlineBuild.Cli/CandidateStatusCommand.cs#L15)). It resolves base/head/branch, hashes full-index tracked and cached diffs plus sorted untracked regular files, reports touched paths and dirty/conflict state, and validates any lease manifest against the requested ticket. Missing refs, conflicted trees, unreadable paths, untracked directories, and symlink/reparse-point inputs fail closed.

### `build plan <ticket-id> [ticket-id ...] [--from-brief]` - Functional
Dispatched per-ticket from the sequential multi-ticket loop ([CliApplication.cs:1167-1182](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1167-L1182)) into the `plan` branch of `RunTicketVerbBodyAsync` ([CliApplication.cs:1450-1498](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1450-L1498)). Investigates a `Backlog` ticket and writes the plan to the ticket description, plus risk/size labels and a `[planned_at: <sha>]` marker comment. Standalone `build plan` ignores `[plan].mode`; only the explicit `--from-brief` flag skips the worker and promotes the Backlog ticket to Ready using the description already on the ticket.
...
The `ship` branch of `RunTicketVerbBodyAsync` ([CliApplication.cs:1619-1716](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1619-L1716)). Deterministic phase, no worker subprocess. Resolves the merge target (`[work].target_branch` if set, else `[ship].base_branch` - `ShipOptions.TargetBranch` resolution at [ShipPhase.cs:251](../../src/ThroughlineBuild.Phases/ShipPhase.cs#L251)), fetches, ancestry-checks and (on clean divergence) auto-rebases, scans for conflict markers, runs `ship.regression_checks`, fast-forward-merges the feature branch, pushes the target unless pushing is disabled (`--no-push` or `[ship] push = false`, threaded as `ShipOptions.NoPush` at [CliApplication.cs:1629](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1629)), posts `[shipped_at: <sha>]`, transitions `InReview -> Done`, decrufts the worktree.
...
- **`--debug`** makes ship verbose (streams git output and full check results to stderr via the `verbose` constructor flag, [CliApplication.cs:1636](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1636)); phase-level progress lines print regardless.
- **Exit codes:** mapped from `ShipFailureStage` at [CliApplication.cs:1704-1714](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1704-L1714): 0 success or post-success decruft warning; 1 rebase / conflict-marker / regression; 4 state-check / fetch / FF-merge infra.
...
Single ticket only (multi rejected at [CliApplication.cs:192-196](../../src/ThroughlineBuild.Cli/CliApplication.cs#L192-L196)); the `rework` branch at [CliApplication.cs:1234-1302](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1234-L1302). Re-implements a ticket whose last `Verdict` was `Rework`, retrieving feedback from the event log via `ReviewFeedbackRetriever` (or `--feedback`).

- **Exit codes:** mapped from `ReworkOutcome` at [CliApplication.cs:1285-1292](../../src/ThroughlineBuild.Cli/CliApplication.cs#L1285-L1292): 0 Implemented, 2 TicketNotInProgress, 3 NoFeedbackAvailable, 4 ImplementFailed.
...
`CloseCommand` ([CloseCommand.cs](../../src/ThroughlineBuild.Commands/CloseCommand.cs)). Translates the reason via `ReasonTranslator`, posts a `wontfix:` comment, transitions `-> Cancelled`, attempts a parent rollup, decrufts the worktree. **Changed:** a missing LLM secret no longer exits 3 - `WireUpConditionalCommands` falls back to `EchoLlmClient` with a warning and records the reason verbatim ([CliApplication.cs:2252-2262](../../src/ThroughlineBuild.Cli/CliApplication.cs#L2252-L2262)); close/defer/reopen are deterministic state commands and always run.
...
- **Multi-ticket chain still has two overlapping code paths** (`ParallelDispatcher` vs the `SequentialChainDispatcher` fallback); the in-code comment at [CliApplication.cs:2083-2084](../../src/ThroughlineBuild.Cli/CliApplication.cs#L2083-L2084) still marks it transitional (TLB-312).
...
Twenty tracked xUnit projects mirror the solution's source projects. At this refresh the suites contain 2,834 `[Fact]`/`[Theory]` declarations across 323 C# files, all `net10.0` with nullable enabled. The public Claude facade has its own `ThroughlineBuild.ClaudeCode.Tests` project. Every source and test project now has a tracked `packages.lock.json`, and CI restores them in locked mode.
...
The repository tracks [.claude/settings.json.example](../../.claude/settings.json.example) as a safe starting point. SOP installation can also materialize catalog-owned Claude command stubs and Codex skill stubs at the paths declared by the embedded `SopBundleCatalog`; these are repository artifacts whose bytes are checked by `build sop doctor`, not live Plane credentials. Repository facts now live in tracked `.build/config.toml`; ignored `.build/conductor.toml` and `.build/sop-manifest.json` remain local conductor/SOP state.

---

## Loose ends (cross-cutting)

- **Help-surface drift remains:** three help surfaces exist (tier-0/tier-1 registry, help topics, legacy `CliUsage`), and they disagree - `sweep`/`models` are the two missing tier-1 entries. The code registry and command implementations win.
