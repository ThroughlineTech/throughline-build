...
Twenty-six action verbs are reachable: `init`, `settarget`, `user-guide`, `op-doc`, `models`, `sweep`, `list`, `get`, `comments`, `comment`, `transition`, `relate`, `setup`, `amend`, `close`, `defer`, `reopen`, `new`, `scaffold`, `rework`, `decompose`, `plan`, `implement`, `review`, `ship`, and `chain`, plus help and version meta-surfaces. `RunAsync` performs the dispatch ([Program.cs:23](../../src/ThroughlineBuild.Cli/Program.cs#L23)). `init`, `settarget`, `user-guide`, `op-doc`, and `models refresh` dispatch before config load.
...
Resolves one configured-project ticket and prints a human view or a `TicketView` JSON envelope through `CliEnvelopeWriter.ToView` ([CliEnvelopeWriter.cs:100](../../src/ThroughlineBuild.Cli/Json/CliEnvelopeWriter.cs#L100)). The ordinary ticket read does not hydrate live relation edges; use `build relate <id> --list` for that graph.

### `build comments <ticket-id> [--json]` / `build comment <ticket-id> <text|-> [--json]` - Functional

`comments` reads and HTML-to-text normalizes the comment stream; `comment` creates one comment, with `-` reading stdin. Their versioned JSON shapes are `CommentsEnvelope` and `CommentCreatedEnvelope` ([CliEnvelope.cs:110](../../src/ThroughlineBuild.Cli/Json/CliEnvelope.cs#L110)).

### `build transition <ticket-id> <state> [--json]` - Functional

Transitions directly to a named lifecycle state and returns a typed acknowledgement. Backend state validation remains owned by `PlaneTicketingClient.TransitionAsync` ([PlaneTicketingClient.cs:848](../../src/ThroughlineBuild.Plane/PlaneTicketingClient.cs#L848)).
...
- **`sweep` and `models` are missing from the help registry.** `HelpRegistryFactory.Build` declares 24 commands; `sweep` and `models` are not among them, so `build sweep --help` falls back to the tier-0 menu instead of per-command help, and neither appears in the tier-0 listing.
...
Twenty tracked xUnit projects mirror the solution's source projects. At this refresh the suites contain 2,450 `[Fact]`/`[Theory]` declarations across 231 C# files, all `net10.0` with nullable enabled. The public Claude facade has its own `ThroughlineBuild.ClaudeCode.Tests` project.
...
The repository tracks only [.claude/settings.json.example](../../.claude/settings.json.example) as a safe starting point. It does not track live project settings, project command definitions, Plane configuration, ticket configuration, or worktree state under `.claude/`, and `build` does not read the example. The repository's ticket workflow is the `build` CLI configured through the gitignored `.build/config.toml`; any globally installed agent commands are external to this inventory.
