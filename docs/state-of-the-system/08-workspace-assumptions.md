...
- **Chain integration branch (CHANGED).** A parent chain creates one shared worktree on `chain/<parent-id>` (`ChainIntegrationBranch.BranchName`, [src/ThroughlineBuild.Phases/ChainIntegrationBranch.cs:34](../../src/ThroughlineBuild.Phases/ChainIntegrationBranch.cs#L34)). This is no longer a commit-less placeholder: `ParentChainRunner` dispatches each child with `ChainTargetBranch` set to the integration branch ([ParentChainRunner.cs:445](../../src/ThroughlineBuild.Phases/ParentChainRunner.cs#L445)), refreshes a reused branch against the base before dispatch (TLB-546, :120), and invokes `LandRootIntegrationBranchAsync` for the outermost chain (:548). Chain success removes the integration worktree but retains the branch; `build sweep` is the merged-gated branch-deletion path (see 05).
...
- **Chain preflight wrong-branch guard (NEW).** The outermost chain refuses up front when the main worktree is not checked out on the target branch (or detached): `ChainPreflight` emits `chain_preflight_wrong_branch` and returns `RefusedWrongBranch`, exit 2 ([src/ThroughlineBuild.Phases/ChainPreflight.cs:45](../../src/ThroughlineBuild.Phases/ChainPreflight.cs#L45)). It mirrors ship's own pre-merge guard so the operator fixes the branch before any planning, not after.
- **Ship wrong-branch guard.** `ShipPhase` still verifies the main worktree is on the target before merging - unconditional, including `main` (`wrong_worktree_branch`, [src/ThroughlineBuild.Phases/ShipPhase.cs:260](../../src/ThroughlineBuild.Phases/ShipPhase.cs#L260)); `ChainIntegrationBranch.LandRootIntegrationBranchAsync` repeats it as `chain_landing_wrong_branch` ([src/ThroughlineBuild.Phases/ChainIntegrationBranch.cs:119](../../src/ThroughlineBuild.Phases/ChainIntegrationBranch.cs#L119)).
...
- **Preflight gates.** `CheckAsync` rejects unmerged/conflicted paths and dangling stash entries unrelated to the ticket branch ([src/ThroughlineBuild.Phases/WorkingTreeHygieneGate.cs:24](../../src/ThroughlineBuild.Phases/WorkingTreeHygieneGate.cs#L24)). It runs before implement (`GateFailure` kind `hygiene_gate`, [src/ThroughlineBuild.Phases/ImplementPhase.cs:139](../../src/ThroughlineBuild.Phases/ImplementPhase.cs#L139)) and once through `ChainPreflight` at outermost chain start (`hygiene_gate_preflight` -> `RefusedDirtyTree`, [src/ThroughlineBuild.Phases/ChainPreflight.cs:63](../../src/ThroughlineBuild.Phases/ChainPreflight.cs#L63)); chain preflight also refuses tracked changes in the main checkout as `chain_preflight_dirty` (:84; untracked ignored). `ShipPreflightAsync` checks both worktrees plus the repo-global stash ([WorkingTreeHygieneGate.cs:83](../../src/ThroughlineBuild.Phases/WorkingTreeHygieneGate.cs#L83), invoked at [src/ThroughlineBuild.Phases/ShipPhase.cs:209](../../src/ThroughlineBuild.Phases/ShipPhase.cs#L209)).
