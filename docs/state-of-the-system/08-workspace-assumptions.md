...
- **Base / target branch.** `main` by default. The merge destination is resolved by `BuildConfig.ResolveTargetBranch()` = `[work].target_branch ?? [ship].base_branch` ([src/ThroughlineBuild.Cli/Config.cs:58](../../src/ThroughlineBuild.Cli/Config.cs#L58)). `BaseRefResolver.ResolveAsync` is now **target-aware**: it takes the resolved target branch and tries `<remote>/<target>` then falls back to the local `<target>` ([src/ThroughlineBuild.Git/BaseRefResolver.cs:13-25](../../src/ThroughlineBuild.Git/BaseRefResolver.cs#L13-L25)) - the branch is passed in, no longer a literal `main`. The resolved target flows into `ShipOptions.TargetBranch` and `BuildOptions.TargetBranch`. Set the override with `build settarget <branch>`.
...
- **`BaseRefResolver` is now target-parameterized**, but the `remote` is still the literal `origin/` prefix inside it ([src/ThroughlineBuild.Git/BaseRefResolver.cs:16](../../src/ThroughlineBuild.Git/BaseRefResolver.cs#L16)); `ship.remote` reaches `ShipPhase` but not the resolver's remote prefix. Phases other than ship resolve against `BuildOptions.TargetBranch` (default `main`).
...
- **The wrong-branch preflight only fires for non-default targets.** When `target == base_branch` the guard is skipped, so shipping to `main` does not require the main worktree to be on `main` first.
...
- **`BaseRefResolver` resolves a passed-in target branch** but hardcodes the `origin/` remote prefix ([src/ThroughlineBuild.Git/BaseRefResolver.cs:16](../../src/ThroughlineBuild.Git/BaseRefResolver.cs#L16)); plan/implement/review/decompose resolve against `BuildOptions.TargetBranch` (default `main`), while `ship` uses the fully-resolved `[work].target_branch`/`[ship]` settings. A repo whose remote is not `origin` would still mis-resolve the base ref outside ship.
