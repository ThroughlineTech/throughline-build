...
5. Resolves both the current worktree root and the clone's main worktree through `RepositoryLayout.Resolve`; tracked `.build` policy stays relative to the current tree while an older tree without `.build` can fall back to the main tree ([RepositoryLayout.cs:59](../../src/ThroughlineBuild.Cli/RepositoryLayout.cs#L59)).
...
| Conductor policy | `.build/conductor.toml` (tracked) | Edit through `build conductor apply` or normal reviewed repository changes. |
| SOP cache | `.build/sop-manifest.json` (ignored; [.gitignore:3](../../.gitignore#L3)) | Delete for a local reset. Prefer `build sop uninstall` first so hash-matching catalog stubs are safely removed. |
