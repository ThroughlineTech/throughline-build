...
- **Git**: the feature branch `ticket/<id>` (id only since TLB-408) and its worktree at `.worktrees/ticket-<id>/`; ship fetches, auto-rebases the local target branch onto `<remote>/<target>` on a clean divergence, fast-forward-merges the feature branch in, and pushes (unless `--no-push`/`[ship].push=false`, which keeps the merge fully local). The merge target defaults to `[ship].base_branch` (`main`) but can be redirected to a feature integration branch via `[work].target_branch` (set with `build settarget`); ship preflight-guards that the main worktree is actually on that branch - now unconditionally, catching a detached HEAD even when shipping to `main` ([src/ThroughlineBuild.Phases/ShipPhase.cs:256-277](../../src/ThroughlineBuild.Phases/ShipPhase.cs#L256-L277)). A chain runs its children inside one shared worktree on a placeholder `chain/<slug>` branch.
