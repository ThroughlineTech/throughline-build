...
| make a ticket | assemble a JSON draft, then `build new - --json` (fields: `title`, `type`, `description`, `acceptanceCriteria`, `labels`, `parent`, `relations` as `[{"kind":"blocked_by","targetId":"TLB-N"}]`; markdown is rendered to HTML) |
