...
stdout by REVERSE scan, so the last envelope wins; the payload is the first
complete JSON value after the marker (trailing narration is ignored). Metadata
is an AOT-safe `Dictionary<string, JsonElement>`. A fenced-block pre-pass
captures `<<<NAME_START` / `<<<NAME_END` payload blocks emitted before the
LAST envelope marker (duplicate names are last-wins);
