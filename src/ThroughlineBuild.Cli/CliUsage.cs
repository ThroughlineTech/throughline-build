...
Usage:
  ...
  build init [--force] [--print-template] [--from FILE] [--plane-url URL] [--workspace SLUG] [--project-id UUID] [--project-name NAME] [--token TOKEN | --token-env VAR]   Write .build/config.toml from the built-in template; --force overwrites an existing file; --print-template prints to stdout; --from FILE reads credentials from a key=value file (plane_base_url, plane_workspace_slug, plane_api_token, plane_project_id, plane_project_name); when stdin is redirected and --from is absent, stdin is read as a credentials file; explicit flag values take precedence over file values; when stdin is a TTY and no --from is given, prompts for any REQUIRED_ values not supplied as flags; when --project-name (or plane_project_name in the creds file) is given together with full credentials, connected mode runs: resolves or creates the Plane project, writes the resolved id into the config, provisions states/labels, verifies connectivity, and prints a summary
