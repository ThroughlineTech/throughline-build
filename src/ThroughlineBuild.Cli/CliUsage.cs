...
Usage:
  ...
  build chain <ticket-id> [ticket-id ...] [--agent <name>] [--agent-plan <name>] [--agent-implement <name>] [--agent-review <name>] [--debug] [--no-auto-resolve] [--no-auto-merge]  Run the full chain for one or more tickets; multi-ticket dispatch is level-synchronous (dependency-ordered, concurrency bounded by workers.max_concurrency); streams per-phase output to stdout
