...
Usage:
  ...
  build setup [--check]        Make the configured Plane project meet workflow criteria: create any missing states (Backlog, Planning, Ready, In Progress, In Review, Done, Cancelled) and labels (risk:*, size:*, plan-ticket, stub, delegated). Idempotent. --check verifies only and exits 1 when the project does not meet criteria (creates nothing). Run after 'build init', before the first 'build new'/'build chain'.
