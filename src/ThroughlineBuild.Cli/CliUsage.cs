...
Usage:
  ...
  build chain <ticket-id> [--agent <name>] [--agent-plan <name>] [--agent-implement <name>] [--agent-review <name>] [--debug] [--no-auto-resolve]  Run the full chain (plan -> implement -> review -> ship cycle) for a ticket (multi-ticket support is in development); streams per-phase output to stdout
