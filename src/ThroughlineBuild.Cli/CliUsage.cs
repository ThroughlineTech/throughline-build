...
Usage:
  ...
  build ship <ticket-id> [--debug] [--summary-json] [--no-auto-merge]               Ship a reviewed ticket (local fast-forward merge; no push to remote); --debug accepted but is a no-op (ship has no worker subprocess)
  build chain <ticket-id> [--agent <name>] [--agent-plan <name>] [--agent-implement <name>] [--agent-review <name>] [--debug] [--no-auto-resolve] [--no-auto-merge]  Run the full chain (plan -> implement -> review -> ship cycle) for a ticket (multi-ticket support is in development); streams per-phase output to stdout
