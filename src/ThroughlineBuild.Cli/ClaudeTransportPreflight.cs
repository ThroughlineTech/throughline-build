...
public static class ClaudeTransportPreflight
{
    ...
    /// agents. A standalone plan is gated unless <c>--from-brief</c> explicitly requests promotion;
    /// configured promote mode applies only to chain. Ship and any non-phase verb return empty.
    /// Chain's <c>implement</c> entry also covers the batch-implement worker (built from the
    /// implement-phase agent). The draft (<c>build new</c>) and scaffold paths gate at their own
    /// dispatch sites and are not routed here.
    /// </summary>
    public static IReadOnlyList<string> PhasesToGateForVerb(string verb, bool planIsPromote, bool fromBrief)
    {
        var promotesPlan = PlanDispatchPolicy.ShouldPromote(verb, fromBrief, planIsPromote);
        return verb switch
        {
            ...
            "chain" when promotesPlan => new[] { "implement", "review" },
            "chain" => new[] { "plan", "implement", "review" },
            "plan" when promotesPlan => Array.Empty<string>(),
            "plan" => new[] { "plan" },
