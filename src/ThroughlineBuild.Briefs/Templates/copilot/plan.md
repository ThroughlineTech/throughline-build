...
3. **Deep-dive the code.** Use the Read, Grep, Glob, Bash, and Edit tools aggressively from key source locations. Read relevant files end-to-end; skimming produces vague plans. Map call chains entry -> business logic -> data layer. Identify the interfaces, types, and tests that will be touched.
