...
public static class OpDocValidator
{
    ...
    private static void ValidateBriefNumbering(Plan plan, List<ValidationError> errors)
    {
        ...
        for (int i = 0; i < sortedBriefs.Count; i++)
        {
            int expectedNumber = i + 1;
            if (sortedBriefs[i].Number != expectedNumber)
            {
                // Check if it's a gap
                if (sortedBriefs[i].Number > expectedNumber)
                {
                    errors.Add(new ValidationError(
                        ...
                        Message: $"Brief numbering has a gap. Expected {expectedNumber:D2} but found {sortedBriefs[i].Number:D2}."));
                }
            }
        }
