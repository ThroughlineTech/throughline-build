...
- "role" is "gating", "advisory", or "setup" and is REQUIRED on every check. Classify by what the
  ...
  - "setup": NOT a check - a PREREQUISITE command the engine runs ONCE in the worktree BEFORE the
    gating/advisory checks (every setup step runs first). Use ONLY when build/test genuinely cannot run
    in a fresh checkout without a generate/install step first - e.g. an iOS project whose *.xcodeproj is
    gitignored and regenerated by `xcodegen generate`, or a codegen step the compile depends on. Most
    projects need NO setup step (their deps and build inputs are already present in the worktree); do
    not emit one unless the build fails without it. A failed setup step hard-fails the gate. Shape:
